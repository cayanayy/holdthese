...
const ItemCard = ({item}) => {
    const leftTimeMilis = Date.parse(item.unableAt) - Date.now();
    const day = Math.floor(leftTimeMilis / 86400000);
    const hour = Math.floor((leftTimeMilis % 86400000) / 3600000);
    const minutes = Math.round(((leftTimeMilis % 86400000) % 3600000) / 60000);
    const untilUnable =
        day > 0
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col sm:flex-row">
                {[...item.files].length > 0 ? (
                    <button className="mr-4 font-black" onClick={downloadFiles}>
                        download files
                    </button>
                ) : (
                    <></>
